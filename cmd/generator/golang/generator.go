...
func (g *generator) Tests() (err error) {
	g.printf("// Code generated by benc generator; DO NOT EDIT.\n\n")
	g.printf("package %s\n\n", g.PkgName)
	g.printf("import (\n")
	g.printf("\t\"math/rand\"\n")
...
func (g *generator) formatGo(prefix string) (err error) {
	...
	if _, err = g.buf.Write(src); err!=nil {
		return 
	}

	g.WriteFile(&g.buf, prefix, "go")
...
func (g *generator) getGoSizeExpr(expr ast.Expr, varName string) string {
	...
	case *ast.ArrayType:
		if t.Len != nil {
			// Fixed Array
			lenStr := g.ExprToString(t.Len)
			eltInfo := g.getTypeInfo(t.Elt)
			if eltInfo.TypeName == "byte" {
				return fmt.Sprintf("bstd.SizeByteArray(%s)", lenStr)
			}
			if eltInfo.IsFixedSize {
				// Optimization for fixed size elements (e.g., [5]int64 -> 5 * 8)
				return fmt.Sprintf("(%s * %s)", lenStr, g.getGoSizeExpr(t.Elt, "(*new("+eltInfo.TypeName+"))"))
			}
			// Dynamic size elements (e.g., [5]string)
			eltSizer := fmt.Sprintf("func(v %s) int { return %s }", eltInfo.TypeName, g.getGoSizeExpr(t.Elt, "v"))
			return fmt.Sprintf("bstd.SizeArray(%s[:], %s)", varName, eltSizer)
		}
...
func (g *generator) getGoMarshalExpr(expr ast.Expr, n, buf, varName string) string {
	...
	case *ast.ArrayType:
		if t.Len != nil {
			...
			eltInfo := g.getTypeInfo(t.Elt)
			if eltInfo.TypeName == "byte" {
				return fmt.Sprintf("bstd.MarshalByteArray(%s, %s, %s[:])", n, buf, varName)
			}
			eltMarshaler := fmt.Sprintf("func(n int, b []byte, v %s) int { return %s }", eltInfo.TypeName, g.getGoMarshalExpr(t.Elt, "n", "b", "v"))
			return fmt.Sprintf("bstd.MarshalArray(%s, %s, %s[:], %s)", n, buf, varName, eltMarshaler)
		}
...
func (g *generator) getGoUnmarshalExpr(expr ast.Expr, n, buf, varName string) string {
	...
	case *ast.ArrayType:
		if t.Len != nil {
			...
			eltInfo := g.getTypeInfo(t.Elt)
			if eltInfo.TypeName == "byte" {
				return fmt.Sprintf("n, err = bstd.UnmarshalByteArray(%s, %s, %s[:])", n, buf, varName)
			}
			...
			return fmt.Sprintf("n, err = bstd.UnmarshalArray[%s](%s, %s, %s[:], %s)", eltInfo.TypeName, n, buf, varName, eltUnmarshaler)
		}
...
func (g *generator) getTypeInfo(expr ast.Expr) typeGenInfo {
	...
	case *ast.ArrayType:
		...
		if t.Len != nil {
			...
			// Comparer
			cmpCall := eltInfo.TestComparer
			testCmp := fmt.Sprintf("func(a, b %s) error { return btst.CompareSlice(a[:], b[:], %s) }", typeName, cmpCall)

			return typeGenInfo{
				TypeName: typeName,
				TestGenerator: testGen,
				TestComparer: testCmp,
	...
	case *ast.MapType:
		...
		comparerFunc := "btst.CompareMap"
		if _, isPtr := t.Key.(*ast.StarExpr); isPtr {
			comparerFunc = "btst.ComparePointerKeyMap"
		}
		...
		return typeGenInfo{
			...
			// Use the dynamic comparer func
			TestComparer:  fmt.Sprintf("func(a, b %s) error { return %s(a, b, %s) }", typeName, comparerFunc, valInfo.TestComparer),
		}
	}
